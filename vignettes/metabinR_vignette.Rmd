...
abundance_based_binning(
    ...
    numOfClustersAB = 2, kMerSizeAB = 10, outputAB = "AB.cluster")
```
...
```{r, eval=TRUE, message=FALSE, warning=FALSE}
histogram <- read.table("AB.cluster__histogram.tsv", header = TRUE)
ggplot(histogram, aes(x=counts, y=frequency)) + geom_area() +
    labs(title = "kmer counts histogram") + theme_bw()
```

We get the assigned abundance class for each read with :
```{r, eval=TRUE, message=FALSE, warning=FALSE}
abundance.clusters <- 
    rbind(
        data.frame(read_id=readLines("AB.cluster__001.fasta")[c(TRUE,FALSE)],
                    AB_id=1),
        data.frame(read_id=readLines("AB.cluster__002.fasta")[c(TRUE,FALSE)],
                    AB_id=2))
abundance.clusters$read_id <- sub('.', '', abundance.clusters$read_id)
abundance.clusters <- abundance.clusters[order(abundance.clusters$read_id),]
```

Evaluate predicted abundance class and plot confusion matrix :
```{r, eval=TRUE, message=FALSE, warning=FALSE}
eval <- cvms::evaluate(
    data = data.frame(prediction=as.character(abundance.clusters$AB_id),
                        target=as.character(reads.mapping$AB_id),
                        stringsAsFactors = FALSE),
    prediction_cols = "prediction", target_col = "target", type = "binomial"
)
p <- cvms::plot_confusion_matrix(eval) +
    labs(title = "Confusion Matrix", 
        x="Target Abundance Class",y="Predicted Abundance Class")
tab <- as.data.frame(
    c(
        Accuracy =  round(eval$Accuracy,4),
        Specificity =  round(eval$Specificity,4),
        Sensitivity =  round(eval$Sensitivity,4),
        Fscore =  round(eval$F1,4),
        Kappa =  round(eval$Kappa,4)
    )
)
